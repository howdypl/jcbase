...
public class AuthorityInterceptor implements Interceptor {
	...
	public void intercept(Invocation inv) {
		...
		if(inv.getActionKey().contains("download")||inv.getActionKey().contains("app/update")){
			hasPermissions = true;
		}else if(inv.getActionKey().contains("mobile")){
			SysUser sysUser=IWebUtils.getCurrentSysUser(inv.getController().getRequest());
			if(sysUser==null){//没有登陆
				hasPermissions=false;
				InvokeResult result = InvokeResult.failure("用户未登录，请先登录");
