...
<%
	...
	String baseAppPath = baseImagePath+"app"+"/"; %>
<!DOCTYPE html>
<html lang="en">
	<head>
	...
	<body class="no-skin">
        ...
        $(document).ready(function () {
            ...
            $("#grid-table").jqGrid({
                url:'${context_path}/updateapp/getListData',
                mtype: "GET",
                datatype: "json",
                postData:{'username':username},
                colModel: [
                	{ label: '应用编码', name: 'code', width: 32 },
                	{ label: '应用包', name: 'uploadurl', width: 35 },
                	{ label: '发布时间', name: 'send_time', width: 30 },
                	{ label: '应用信息', name: 'msg', width: 60 },
                ],
				viewrecords: true,
                height: 500,
                rowNum: 10,
            ...
            {
                if (cellvalue != "0"){
                	return '<a href="<%=baseAppPath%>' + cellvalue + '">下载</a>'
                    // return '<img class="alarmimg" src="<%=baseAppPath%>' + cellvalue + '" alt="' + cellvalue + '" style="width:150px;height:80px;" />';
                }
            }
			...
			$("#btn-add").click(function(){//添加页面
				parent.layer.open({
				    ...
				    fix: false, //不固定
				    maxmin: true,
				    content: '${context_path}/updateapp/add'
				});
			});
