...
//resize on sidebar collapse/expand
            ...
            $("#grid-table").jqGrid({
                url:'${context_path}/setpoint/getListData',
                mtype: "GET",
                datatype: "json",
                colModel: [
                	{ label: '预设点名称', name: 'platform_code', width: 40 },
                	{ label: '设定温度值', name: 'warn_temp', width: 30 },
                    { label: '监控器', name: 'pp_sensor_code', width: 60 ,sortable:false},
                    { label: '图片',name: 'images', width: 100, align: "center", sortable: false, editable: false, formatter: alarmFormatter},
                    { label: '注册时间', name: 'create_time', width: 50 },
                ],
				viewrecords: true,
                height: 280,
                rowNum: 10,
			...
			$("#btn-edit").click(function(){//修改页面
				...
				if(rid == -1){
					...
					parent.layer.open({
					    ...
					    fix: false, //不固定
					    maxmin: true,
					    content: '${context_path}/setpoint/add?id='+rid
					});
				}
			});
        });
