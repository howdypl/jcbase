														...
														<div class="row">
															<div class="col-xs-12 col-sm-8">
																<div class="input-group">
																	...
																	<input type="text" id="name" name="name" class="form-control search-query" placeholder="请输入工区名称的关键字" />
...
<%-- 										<jc:button className="btn" id="bnt-grant" textName="资源授权" permission="/sys/role"/> --%>
        ...
        $(document).ready(function () {
            ...
            $("#grid-table").jqGrid({
                url:'${context_path}/workarea/getListData',
                mtype: "GET",
                datatype: "json",
                colModel: [
                	{ label: '工区名称', name: 'area', width: 60 },
                    { label: '地址', name: 'wa_addr', width: 75 ,sortable:false},
                    //{ label: '负责人', name: 'opNames', width: 100 },
                    { label: '描述', name: 'wa_desc', width: 150 ,sortable:false},
			...
			$("#btn-add").click(function(){//添加页面
				parent.layer.open({
				    ...
				    fix: false, //不固定
				    maxmin: true,
				    content: '${context_path}/workarea/add'
				});
			});
...
// 			});
			$("#btn-unvisible").click(function(){
				...
				$.post("${context_path}/workarea/deleteData", submitData,function(data) {
					if (data.code ==0) {
						layer.msg("删除成功", {
	        				icon: 1,
	        			    time: 2000 //2秒关闭（如果不配置，默认是3秒）
			...
			$("#btn-edit").click(function(){//添加页面
				...
				if(rid == -1){
					...
					parent.layer.open({
					    ...
					    fix: false, //不固定
					    maxmin: true,
					    content: '${context_path}/workarea/add?id='+rid
					});
				}
			});
        });
